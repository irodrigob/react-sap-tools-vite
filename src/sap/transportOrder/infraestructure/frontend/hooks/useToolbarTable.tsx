...
export default function useToolbarTable() {
	...
	const handlerTextSearch = useCallback(
		() => (event: Ui5CustomEvent<InputDomRef, never>) => {
			...
			if (
				...
				setOrderTaskSelectedAction([]);
				setRowsExpandedAction([]);
				setTextSearchOrdersAction(event.target.value as string);
			}
		},
		[]
	);
	...
	const handlerTransportCopy = useCallback(() => {
		if (orderTaskSelected.length > 0) {
			setDescriptionTransportCopyAction(orderTaskSelected[0].description);
	 ...
	 */
	const handlerDoNewOrder = useCallback((data: FormNewOrder) => {
	...
	const handlerOrderObjects = useCallback(() => {
		if (orderTaskSelected.length > 0) {
			setShowOrderObjectsAction(true);
			setOrdersObjectsSelectedAction(
				orderTaskSelected
